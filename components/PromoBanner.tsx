...
const PromoBanner: React.FC<PromoBannerProps> = () => {
  ...
  const buttonClasses =
    'bg-white text-red-600 px-6 py-2 rounded-lg hover:bg-gray-100 transition-colors duration-200 font-semibold';
              ...
              <div className="flex-1 flex justify-center md:justify-end items-center p-4 md:p-8">
                <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                  {[1, 2, 3, 4, 5, 6].map((i: number) => (
                    <div
                      key={i}
                      className="w-10 h-10 md:w-12 md:h-12 bg-red-400 rounded opacity-50"
                    ></div>
                  ))}
                </div>
              </div>
